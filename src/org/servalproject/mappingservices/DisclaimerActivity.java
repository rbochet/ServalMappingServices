...
public class DisclaimerActivity extends Activity implements OnClickListener {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// bind to the service
		bindService(new Intent(this, MapDataService.class), connection,
				Context.BIND_AUTO_CREATE);

		// get the phone number and sid from the Serval sticky
		BatphoneBroadcast mBroadcast = new BatphoneBroadcast(
				this.getApplicationContext());
		IntentFilter mBroadcastFilter = new IntentFilter(
				"org.servalproject.SET_PRIMARY");
		// mBroadcastFilter.addDataScheme("tel");
		this.getApplicationContext().registerReceiver(mBroadcast,
				mBroadcastFilter);
	}
	...
	public void onClick(View v) {
		...
		if (v.getId() == R.id.btn_disclaimer_continue) {
			...
			// determine what action to take based on the number of files
			// available
			if (mapFileName == null && mapFileNames == null) {
				// show a dialog asking for confirmation to continue when no map
				// files are available
	...
	protected Dialog onCreateDialog(int id) {
		...
		case DIALOG_OK_CONTINUE:
			...
			mBuilder.setMessage(this
					.getString(R.string.disclaimer_no_map_file_msg));
			mBuilder.setCancelable(false);

			mBuilder.setPositiveButton(this.getString(R.string.map_alert_yes),
			...
			mBuilder.setNegativeButton(this.getString(R.string.map_alert_no),
					new DialogInterface.OnClickListener() {
						public void onClick(DialogInterface dialog, int id) {
							dialog.cancel();
						}
	...
	private void showMapActivity(String mapFileName) {
		...
		Intent mServiceIntent = new Intent(
				this,
				org.servalproject.mappingservices.services.CoreMappingService.class);
		startService(mServiceIntent);

		// show the map
		Intent mMapIntent = new Intent(this,
				org.servalproject.mappingservices.MapActivity.class);
	...
	private class IncomingHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			...
			case MapDataService.MSG_FILE_LIST:
				...
				/*
				 * 
				 * if(mBundle.getInt("fileCount") > 0) { // check to see if the
				 * list contains the file we expect boolean mFound = false;
				 * String[] mFileList = mBundle.getStringArray("fileList");
				 * 
				 * 
				 * // // for(int i = 0; i < mFileList.length; i++) { //
				 * if(mFileList[i].equals(MapActivity.MAP_DATA_FILE) == true) {
				 * // mFound = true; // continue; // } // } // if(mFound ==
				 * false) { //TODO be gentler about the fact that map data
				 * cannot be found Toast.makeText(self.getApplicationContext(),
				 * String
				 * .format(self.getString(R.string.disclaimer_missing_map_file_error
				 * ), MapActivity.MAP_DATA_FILE) , Toast.LENGTH_LONG).show();
				 * self.onActivityResult(0, 0, null); } } else { // show toast
				 * and exit //TODO be gentler about the fact that map data
				 * cannot be found Toast.makeText(self.getApplicationContext(),
				 * R.string.disclaimer_no_map_file_error,
				 * Toast.LENGTH_LONG).show(); self.onActivityResult(0, 0, null);
				 * }
				 */
			default:
	...
	/*
	 * ServiceConnection class that represents a collection to the
	 * MappindDataService
	 */
	private ServiceConnection connection = new ServiceConnection() {
		...
		public void onServiceConnected(ComponentName className, IBinder service) {
			...
			try {
				...
				Message msg = Message
						.obtain(null, MapDataService.MSG_FILE_LIST);
	...
	private class BatphoneBroadcast extends BroadcastReceiver {
		...
		/**
		 * construct a new BatphoneBroadcast class which receives the sticky
		 * broadbast from the serval batphone software
		 * 
		 * @param context
